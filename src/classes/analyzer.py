...
def select_best_project(projects: list[ProjectFiles]) -> ProjectFiles:
    """
    Select the best project from a list of discovered projects.
    Prioritizes actual source code over infrastructure/build artifacts.
    """
    if not projects:
        raise ValueError("No projects provided for selection")
    ...
    for project in projects:
        ...
        if any(keyword in project.name.lower() for keyword in skip_keywords):
            continue

        # Count source code files using existing analyzer logic
        source_file_count = 0
        for file_path in project.file_paths:
            try:
                full_path = os.path.join(project.root_path, file_path)
                analyzer = get_appropriate_analyzer(full_path)

                # Check if it's a code analyzer (source code)
                if analyzer.__class__.__name__ in {'PythonAnalyzer', 'JavaAnalyzer', 'JavaScriptAnalyzer',
                                                  'HTMLAnalyzer', 'CSSAnalyzer', 'CodeFileAnalyzer'}:
                    source_file_count += 1
            except:
                continue

        if source_file_count > 0:
            source_ratio = source_file_count / len(project.file_paths)
            # Prefer projects with high source ratios and reasonable sizes
            score = source_ratio * 100 - (len(project.file_paths) / 100 if len(project.file_paths) > 1000 else 0)
            source_candidates.append((project, score))

    if source_candidates:
        # Return project with highest score
        best_project = max(source_candidates, key=lambda x: x[1])[0]
        logger.info(f"Selected source code project: {best_project.name}")
        return best_project

    # Fallback to first project
    logger.warning("Could not identify source code project. Using first project as fallback.")
    return projects[0]